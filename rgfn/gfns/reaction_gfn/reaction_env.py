...
class ReactionEnv(EnvBase[ReactionState, ReactionActionSpace, ReactionAction]):
    ...
    def _enumerate_decompositions(
        ...
        """
        Enumerate ALL valid backward trajectories that decompose a molecule into fragments.

        Returns a list of trajectories in the form
        [fragment_smiles, (reaction, fragment1, fragment2,...), intermediate_smiles, ..., Molecule]
        """

        # Base case: molecule is already a fragment
        if (
            self._is_fragment(molecule.smiles)
            and self.fragments[self.smiles_to_fragment_idx[molecule.smiles]].num_reactions
            == n_reactions
            ...
            return [
                [molecule.smiles]
            ]  # Empty trajectory - successfully decomposed to initial state s_0

        # Base case 2: n_reactions exhausted, should be a fragment but isn't
        if n_reactions == 0:
            return []  # No valid trajectories

        # Base case 3: invalid molecule
        if not molecule.valid:
            return []

        all_trajectories = []

        # Decompose the molecule by all reactions possible
        for reaction, disconnection in zip(self.reactions, self.disconnections):
            reactants_list = disconnection.rdkit_rxn.RunReactants((molecule.rdkit_mol,))
            ...
            for reactants in reactants_list:
                ...
                # Skip invalid decompositions
                if len(fragments) + len(non_fragments) == 0 or len(non_fragments) > 1:
                    continue

                # If we get to this point, we have <= 1 non-fragment, and >= 1 fragments + non-fragment.
                # Then, we recursively decompose all of these reactants.
                # If we have a non-fragment and no more reaction budget, we've reached an invalid state.

                # Number of reactions needed to make fragments used in this reaction step
                subtree_reactions = sum(f.num_reactions for f in fragments)

                # Case 1: All reactants are fragments
                if len(non_fragments) == 0:
                    for fragment in fragments:
                        # How many reactions remain after accounting for this fragment's subtree?
                        subtree_remaining = subtree_reactions - fragment.num_reactions

                        # Recursively get all ways to decompose this fragment
                        sub_trajectories = self._enumerate_decompositions(
                            fragment, n_reactions - 1 - subtree_remaining, max_trajectories
                        )

                        for sub_traj in sub_trajectories:
                            # Create reaction tuple: (reaction, fragment1, fragment2)
                            other_fragments = [f for f in fragments if f != fragment]
                            reaction_tuple = (reaction,) + tuple(f.smiles for f in other_fragments)

                            # Build trajectory: [fragment_smiles, (reaction_tuple), intermediate, ...]
                            new_trajectory = sub_traj + [reaction_tuple, molecule.smiles]
                            all_trajectories.append(new_trajectory)

                            # Early termination if we hit max_trajectories
                            if max_trajectories and len(all_trajectories) >= max_trajectories:
                                return all_trajectories

                # Case 2: One non-fragment to decompose
                elif self._is_decomposable(non_fragments[0], n_reactions - subtree_reactions - 1):
                    sub_trajectories = self._enumerate_decompositions(
                        non_fragments[0], n_reactions - subtree_reactions - 1, max_trajectories
                    )

                    for sub_traj in sub_trajectories:
                        # Create reaction tuple: (reaction, fragments)
                        reaction_tuple = (reaction,) + tuple(f.smiles for f in fragments)
